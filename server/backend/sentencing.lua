...
ps.registerCallback(resourceName .. ':server:sendToJail', function(source, payload)
    ...
    ps.notify(src, locale('Sentencing.SendToJail'), {time = sentence}, 'success')
...
ps.registerCallback(resourceName .. ':server:giveCitation', function(source, payload)
    ...
    ps.notify(playerSrc, locale('Sentencing.PaidFine'), {amount = fine}, 'error')
    ps.notify(src, locale('Sentencing.FinePaid'), {amount = fine}, 'success')
