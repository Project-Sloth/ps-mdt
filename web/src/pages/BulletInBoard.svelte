<script lang="ts">
    ...
    type BulletinCategory = 'announcement' | 'operations' | 'training' | 'general';
    ...
    const STATIC_ALL_CATEGORY: SidebarCategory = { label: 'All Posts', icon: 'dashboard', value: 'all' };

    const DEFAULT_DYNAMIC_CATEGORIES: SidebarCategory[] = [
        { label: 'Announcements', icon: 'campaign',     value: 'announcement' },
        { label: 'Operations',    icon: 'local_police', value: 'operations'   },
        { label: 'Training',      icon: 'school',       value: 'training'     },
        { label: 'General',       icon: 'forum',        value: 'general'      },
    ];
    ...
    let dynamicCategories = $state<SidebarCategory[]>(DEFAULT_DYNAMIC_CATEGORIES);
    ...
    let sidebarCategories = $derived([STATIC_ALL_CATEGORY, ...dynamicCategories]);

    let categoryLabels = $derived(
        Object.fromEntries(dynamicCategories.map(c => [c.value, c.label]))
    );
    ...
    function defaultPost(): Partial<BulletinPost> {
        return { title: '', content: '', category: 'general', priority: 'normal', pinned: false };
    }
    ...
    function canEdit(post: BulletinPost): boolean {
                ...
                >
                    ...
                    <div class="cat-info">
                        ...
                        <span class="cat-count">
                            {postCountFor(cat.value)} post{postCountFor(cat.value) !== 1 ? 's' : ''}
                        </span>
                    </div>
                </button>
            {/each}
                    ...
                    {#each pinnedPosts as post (post.id)}
                        <div class="post-card pinned" class:expanded={expandedId === post.id}>
                            ...
                            <div class="post-meta">
                                ...
                                <span class="material-icons meta-icon">label</span>
                                <span class="meta-cat">{categoryLabels[post.category] ?? post.category}</span>
                                <span class="meta-sep">·</span>
...
<!-- ════════════════════════════════════════════════════════════
     Create / Edit Modal  (Weapons-style)
═════════════════════════════════════════════════════════════ -->
{#if modal.open}
...
<!-- ════════════════════════════════════════════════════════════
     Delete Confirm Modal  (Weapons-style)
═════════════════════════════════════════════════════════════ -->
...
<style>
    ...
    .category-item.active { background: var(--accent-10); border-color: var(--accent-15); }
    ...
    .create-btn {
        ...
        border-radius: 6px;
        color: var(--accent-80, rgba(100,160,255,0.9));
        font-size: 12px;
        font-weight: 600;
        cursor: pointer;
        transition: all 0.15s;
    ...
    .post-card.pinned {
        background: linear-gradient(135deg,
            ...
            rgba(var(--accent-rgb, 59,130,246), 0.01)
        );
        border-color: rgba(var(--accent-rgb, 59,130,246), 0.14);
    }

    .post-card.expanded { border-color: rgba(255, 255, 255, 0.12); }
    ...
    .pin-icon    { font-size: 14px; color: var(--accent-50, rgba(59,130,246,0.5)); }
    ...
    .meta-cat  { font-size: 11px; color: rgba(255, 255, 255, 0.35); }
    ...
    .post-body :global(h1),
    .post-body :global(h2),
    .post-body :global(h3) { color: rgba(255, 255, 255, 0.9); margin: 12px 0 6px; }
    ...
    .post-body :global(ul),
    .post-body :global(ol) { padding-left: 20px; margin: 5px 0; }
    ...
    .action-btn.delete {
        color: rgba(220, 70, 60, 0.85);
        background: rgba(220, 70, 60, 0.06);
        border-color: rgba(220, 70, 60, 0.18);
        margin-left: auto;
    }
    .action-btn.delete:hover { background: rgba(220, 70, 60, 0.12); }

    /* ════════════════════════════════════════════════════════
       Modals — Weapons.svelte style
    ════════════════════════════════════════════════════════ */
    .modal-backdrop {
        position: fixed;
        inset: 0;
    ...
    .modal {
        background: var(--card-dark-bg, #1a1c22);
        border: 1px solid rgba(255, 255, 255, 0.06);
        border-radius: 6px;
    ...
    .delete-btn {
        ...
        gap: 4px;
        background: rgba(220, 70, 60, 0.06);
        border: 1px solid rgba(220, 70, 60, 0.18);
        border-radius: 3px;
        padding: 4px 12px;
        color: rgba(220, 70, 60, 0.85);
        font-size: 10px;
